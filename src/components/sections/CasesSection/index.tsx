...
const CasesSection = () => {
  return (
    <section id="cases" className="py-16 md:py-24 bg-gray-50 dark:bg-gray-900 relative overflow-hidden">
      {/* Decorative background elements with controlled positioning */}
      ...
      {/* Dot texture background with controlled opacity */}
      <div className="absolute inset-0 dot-texture pointer-events-none" style={{ opacity: '0.1' }}></div>
      
      {/* Decorative shapes with responsive positioning */}
      <div className="decorative-shape shape-circle bg-onesec-primary/20 w-40 h-40 md:w-80 md:h-80 -top-20 -right-20 md:right-0 animate-float pointer-events-none hidden md:block"
        style={{ animationDelay: '1.5s' }}></div>
      <div className="decorative-shape shape-blob bg-onesec-secondary/15 w-48 h-48 md:w-96 md:h-96 -bottom-40 -left-20 md:left-0 animate-rotate-slow pointer-events-none hidden md:block"></div>
      <div className="decorative-shape shape-triangle bg-onesec-accent/15 w-24 h-24 md:w-48 md:h-48 top-1/4 left-1/4 animate-float pointer-events-none hidden md:block"
        style={{ animationDelay: '2s' }}></div>
      
      {/* Background floating elements with responsive behavior */}
      <div className="absolute inset-0 opacity-15 pointer-events-none">
        {[...Array(4)].map((_, i) => <div key={i} className="absolute w-16 h-16 md:w-24 md:h-24 lg:w-40 lg:h-40 rounded-full animate-pulse-light pointer-events-none" style={{
      ...
      {/* Connecting line to next section with proper positioning */}
      ...
      <div className="container mx-auto relative z-10 px-4 md:px-6">
        <div className="text-center mb-8 md:mb-12 select-text">
          ...
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4 md:mb-6 opacity-0 animate-fade-in delay-1">
